...
/*   Updated: 2020/03/01 16:40:32 by dzonda           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int 		ft_sha256_sigma0(uint32_t word)
{
	return (ft_rotr_uint32(word, 6) ^ ft_rotr_uint32(word, 11) ^ ft_rotr_uint32(word, 25));
}

int 		ft_sha256_sigma1(uint32_t word)
{
	return (ft_rotr_uint32(word, 2) ^ ft_rotr_uint32(word, 13) ^ ft_rotr_uint32(word, 22));
}



void			ft_sha256_compress_sigma(uint32_t	state[8], unsigned int i, uint32_t w[64], uint32_t *pw)
{
   const uint32_t k[64] = { 0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
		...
		if (i < 16)
			w[i] = ft_swap_uint32(&pw[i]);
		else if (i < 64)
			w[i] = w[i - 16] + ft_sha256_wsigma0(w[i - 15]) + w[i - 7] + ft_sha256_wsigma1(w[i - 2]);
		s[0] = state[7] + ft_sha256_sigma0(state[4]) + ft_sha256_ch(state[4], state[5], state[6]) + k[i] + w[i];
		s[1] = ft_sha256_sigma1(state[0]) + ft_sha256_maj(state[0], state[1], state[2]);
		state[7] = state[6];
		state[6] = state[5];
		state[5] = state[4];
		state[4] = state[3] + s[0];
...
int			ft_sha256_transform(t_dgst_ctx *ctx)
{
	...
	while (++i < 64)
	{
		ft_sha256_compress_sigma(state, i, w, m);
		// if (i < 16)

...
/*   Updated: 2020/04/10 18:56:08 by dzonda           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_sha512_transform_word(uint64_t *w, const void *data)
{
	...
	while (i < 80)
	{
		s0 = ft_rotr_uint64(w[i - 15], 1) ^ ft_rotr_uint64(w[i - 15], 8) ^ ft_shiftr_uint64(w[i - 15], 7);
		s1 = ft_rotr_uint64(w[i - 2], 19) ^ ft_rotr_uint64(w[i - 2], 61) ^ ft_shiftr_uint64(w[i - 2], 6);
...
static void		ft_sha512_transform_s(uint64_t state[8], uint64_t s[2], uint64_t w, int i)
{
	...
	s[0] = ft_rotr_uint64(state[4], 14) ^ ft_rotr_uint64(state[4], 18) ^ ft_rotr_uint64(state[4], 41);
	ch = (state[4] & state[5]) ^ ((~state[4]) & state[6]);
	s[0] = state[7] + s[0] + ch + k[i] + w;
	s[1] = ft_rotr_uint64(state[0], 28) ^ ft_rotr_uint64(state[0], 34) ^ ft_rotr_uint64(state[0], 39);

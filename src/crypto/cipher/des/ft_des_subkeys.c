...
/*   Updated: 2021/07/21 22:49:37 by dzonda           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
// static void ft_des_subkeys_permute2(subkey_t dst_key, t_uint64 src_key[17]) {
...
//   for (int i = 1; i < 17; ++i) {
//     for (int j = 0; j < 48; ++j) {
//       ft_set_bit(dst_key[i], j, ft_get_bit((const ubyte *)&src_key[i], PC2[j] - 1));
//     }
//   }
// }
...
// typedef uint64_t subkey_t[17]; /* 17 sets of 64 bits */
static void getSubKeys(t_uint64 key, subkey_t ks) {
  ubyte kp[8];
  int i, j;
  ...
  ft_memset(ks, 0, sizeof(subkey_t));
  /* permute 'key' using table PC1 */

...
{	
	...
	{
		ft_dgst(ftssl_dgst->cmd_key, "", FT_DGST_STRING, ftssl_dgst->md, FT_SSL_FALSE); // ft_dgst_string(ftssl_dgst->cmd_key, "", 0, ftssl_dgst->md);
		ftssl_dgst_dispatch_outp(ftssl_dgst->cmd_name, "", 1, ftssl_dgst->md, ftssl_dgst->outp_key);
	}
	else
	{
		ft_dgst(ftssl_dgst->cmd_key, argv[ftssl_dgst->iarg], FT_DGST_STRING, ftssl_dgst->md, FT_SSL_FALSE); // ft_dgst_string(ftssl_dgst->cmd_key, argv[ftssl_dgst->iarg], ft_strlen(argv[ftssl_dgst->iarg]), ftssl_dgst->md);

...
/*   Updated: 2020/02/10 22:50:53 by dzonda      ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int		ft_ssl_dgst_arg(t_ftssl_dgst *ftssl_dgst, char *arg)
{
	...
	if (lstat(arg, &st) == 0)
	{
		if (S_ISREG(st.st_mode))
		{
			...
			return (ft_ssl_dispatch_err(ftssl_dgst->cmd_name, arg, FTSSL_DGST_ERR_DIR));
	}
	else
		return (ft_ssl_dispatch_err(ftssl_dgst->cmd_name, arg, FTSSL_DGST_ERR_FILE));
	return (EXIT_SUCCESS);
}
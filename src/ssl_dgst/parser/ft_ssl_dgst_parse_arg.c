...
/*   Updated: 2020/02/14 13:22:07 by dzonda           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_ssl_dgst_parse_arg(t_ftssl_dgst *ftssl_dgst, int argc, char *argv[])
{
	...
	if (ft_ssl_dgst_parse_opt(ftssl_dgst, argc, argv) == EXIT_FAILURE)
		return (EXIT_FAILURE);
	if (ft_isemptyarg(ftssl_dgst->iarg, argc, ftssl_dgst->md))
	{
		...
		while (ft_isarg(ftssl_dgst->iarg, argc))
		{
      if (ft_isreg(argv[ftssl_dgst->iarg]))
      {
        ...
        ftssl_dgst_dispatch_outp(ftssl_dgst->cmd_name, argv[ftssl_dgst->iarg], 0, ftssl_dgst->md, ftssl_dgst->outp_key);
      }
      else if (ft_isdir(argv[ftssl_dgst->iarg]))
        err = ft_ssl_dispatch_err(ftssl_dgst->cmd_name, argv[ftssl_dgst->iarg], FTSSL_DGST_ERR_DIR);
      else
        err = ft_ssl_dispatch_err(ftssl_dgst->cmd_name, argv[ftssl_dgst->iarg], FTSSL_DGST_ERR_FILE);

...
int ft_ssl_des_exec_handle_salt(t_ssl_des *ctx) {
  ...
  if (randomData < 0) {
    ft_putendl_fd("Something went wrong when generating random number",
                  STDERR_FILENO);
    return (EXIT_FAILURE);
  } else {
    char myRandomData[8];
...
int ft_ssl_des_exec_handle_password(t_ssl_des *ctx) {
  ...
  if (readpassphrase("Verifying - enter des-ecb encryption password: ",
                     passbuf_confirm, sizeof(passbuf_confirm), 0) == NULL)
    return (EXIT_FAILURE);
...
int ft_ssl_des_exec_pre_process(t_ssl_des *ctx) {
  ...
  if (!ft_strlen(ctx->opt.key)) {
    if (!ft_strlen(ctx->opt.pass)) ft_ssl_des_exec_handle_password(ctx);

    if (!ft_strlen(ctx->opt.salt)) ft_ssl_des_exec_handle_salt(ctx);
